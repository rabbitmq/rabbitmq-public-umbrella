...
my $help;
my $Bugzilla_uri='https://bugzilla.rabbitmq.com/xmlrpc.cgi';
my $Bugzilla_login='your-name-here@rabbitmq.com';
...
if (!defined($Bugzilla_password)) {
 $Bugzilla_password = read_password('Bugzilla password:');
}
...
if ($bug_id) {
    foreach my $bugnum (split(/ /, $bug_id)) {
        $soapresult = $proxy->call('Bug.get_bugs', { ids => [$bugnum] });
        _die_on_fault($soapresult);
